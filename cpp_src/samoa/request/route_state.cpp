...
{
    SAMOA_ASSERT(has_primary_partition_uuid());

    // insertion sort - a performance assumption here is
    //  that inputs are already mostly (fully) sorted
...
void route_state::load_route_state(const server::table::ptr_t & table)
{
    ...
    if(has_peer_uuids &&
       _peer_partition_uuids.size() != table->get_replication_factor() - 1)
    {
        std::stringstream err;
        err << "expected exactly " << table->get_replication_factor() - 1;
