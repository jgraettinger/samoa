...
void rolling_hash::commit_record(unsigned actual_value_length,
    // hint returned by previous get()
    offset_t rec_ptr_ptr /*= 0*/)
{
    record * new_rec = (record*)(_region_ptr + _tbl.end);

    if(new_rec->value_length() < actual_value_length)
        throw std::overflow_error("rolling_hash::commit_record(): "
            "actual_value_length > record->value_length()");

    new_rec->set_value_length(actual_value_length);
